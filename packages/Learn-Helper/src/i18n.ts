...
// const { [STORAGE_KEY_LANGUAGE]: storedLanguage } = await browser.storage.local.get([STORAGE_KEY_LANGUAGE])
...
async function activateLanguage() {
  const storedLanguage = await getStoredLanguage()
  const language = storedLanguage || new Intl.Locale(browser.i18n.getUILanguage()).language
