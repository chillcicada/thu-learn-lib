...
import type { ApiError, Language } from 'thu-learn-lib'
import { ContentType, CourseType, SemesterType } from 'thu-learn-lib'
...
import { dataSlice, helperSlice, uiSlice } from './reducers'
...
import { failReasonToString, getStoredCredential, initiateFileDownload, storeCredential } from '@/utils'
...
export function login(username: string, password: string, save: boolean): AppThunk<Promise<void>> {
...
export function refreshIfNeeded(): AppThunk<Promise<void>> {
  return async (dispatch, getState) => {
...
export function refresh(): AppThunk<Promise<void>> {
  return async (dispatch, getState) => {
...
export function syncLanguage(): AppThunk<Promise<void>> {
...
export function updateCourseNames(): AppThunk<void> {
...
export function refreshCardList(): AppThunk<void> {
...
export function downloadAllUnreadFiles(contents: ContentInfo[]): AppThunk<Promise<void>> {
  return async (dispatch, getState) => {
...
export function loadApp(): AppThunk<Promise<void>> {
...
export function tryLoginSilently(): AppThunk<Promise<void>> {
  return async (dispatch) => {
    const credential = await getStoredCredential()
    if (!credential) {
